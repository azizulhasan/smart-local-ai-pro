...
namespace AtlasAiDev\AppService\Pro;
...
class License {
	...
	/**
	 * AtlasAiDev\AppService\Pro\Client.
	 *
	 * @var Client
	 */
	protected $client;
	...
	public function __construct( Client $client ) {
		$this->client = $client;
		$this->option_key = 'AtlasAiDev_' . md5( $this->client->getSlug() ) . '_manage_license';
	...
	public function __admin_notices() {
		...
		if ( ! $this->should_show_notice_on_current_page() ) return;
			...
			<div class="notice notice-error">
				<p><?php
					printf(
						...
						esc_html__( '%1$s You\'re blocking external requests which means you won\'t be able to get %2$s updates. Please add %3$s to %4$s.', 'atlasaidev' ),
						'<b>'. esc_html__( 'Warning!', 'atlasaidev' ).'</b>',
		...
		<div class="notice notice-error">
			<p><?php
				printf(
					...
					esc_html__( 'The %1$s API Key has not been activated, so the %2$s is inactive! %3$s to activate %4$s.', 'atlasaidev' ),
					'<strong>' . esc_attr( $this->client->getName() ) . '</strong>',
					esc_attr( $this->client->getType() ),
					'<a href="' . esc_url( admin_url( 'admin.php?page=' . $this->menu_args['menu_slug'] ) ) . '">'. esc_html__( 'Click here', TTA_PRO_TEXT_DOMAIN ) .'</a>',
					'<strong>' . esc_attr( $this->client->getName() ) . '</strong>'
				);
			?></p>
		</div>
	...
	protected function send_request( $action, $license = [] ) {
		...
		if ( ! in_array( $action, array_keys( $actions ) ) ) {
			return [
				'success' => false,
				'error'   => esc_html__( 'Invalid Request Action.', 'atlasaidev' ),
			];
		}
		// parse license data
		...
		if ( ! $this->validate_license_data( $license ) ) {
			return [
				'success' => false,
				'error'   => esc_html__( 'Invalid/Empty License Data.', 'atlasaidev' ),
			];
		}
		...
		if ( ! is_wp_error( $response ) ) {
			...
			if ( empty( $response ) || ! isset( $response['success'] ) || ( isset( $response['success'] ) && ! $response['success'] ) ) {
				return [
					'success' => false,
					'error'   => isset( $response['error'] ) ? sanitize_text_field( $response['error'] ) : esc_html__( 'Unknown error occurred, Please try again.', 'atlasaidev' ),
	 ...
	 * @return string
	 */
	public function __getLicenceAPI() { // TODO build this api 
		return 'https://atlasaidev.com/?wc-api=wc-am-api';
	}
	...
	private function setAPI_URL() {
		add_filter( $this->client->getSlug() . '_AtlasAiDev_API_URL', [ $this, '__getLicenceAPI' ], 10 );
	}
	...
	private function restoreAPI_URL() {
		remove_filter( $this->client->getSlug() . '_AtlasAiDev_API_URL', [ $this, '__getLicenceAPI' ], 10 );
	}
	...
	public function add_settings_page( $args = array() ) {
		...
		$defaults = [
			...
			'page_title'  => esc_html__( 'Manage License', 'atlasaidev' ),
			'menu_title'  => esc_html__( 'Manage License', 'atlasaidev' ),
	...
	public function menu_output() {
		...
		?>
		<div class="wrap atlasaidev-license-settings-wrapper">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'License Settings', 'atlasaidev' ); ?></h1>
			<hr class="wp-header-end">
			<?php do_action( 'before_atlasaidev_' . $this->client->getSlug() . '_license_section' ); ?>
			<div class="atlasaidev-license-settings atlasaidev-license-section">
				<?php $this->show_license_page_card_header(); ?>
				<div class="atlasaidev-license-details">
					<?php if ( 'activate' == $action ) { ?>
					<p><?php
						/* translators: %s: This Plugin Name */
						printf( esc_html__( 'Active %s by your license key to get professional support and automatic update from your WordPress dashboard.', 'atlasaidev' ), '<strong>' . esc_html( $this->client->getName() ) . '</strong>' );
					?></p>
					<?php } ?>
						...
						<div class="license-input-fields">
							<div class="input-group">
								<div class="license-input code<?php echo $need_prod_id ? ' has-product-id' : '' ?>">
									...
									<label for="license_key" class="screen-reader-text"><?php esc_html_e( 'License Key', 'atlasaidev' ); ?></label>
									<input class="regular-text" id="license_key" type="text"
									       ...
									       placeholder="<?php esc_attr_e( 'Enter your license key to activate', 'atlasaidev' ); ?>"
									       name="<?php echo esc_attr( $this->data_key ); ?>[license_key]"<?php if ( $wp_version < 5.9 ) { readonly( ( 'deactivate' == $action ), true, true ); } else { wp_readonly( ( 'deactivate' == $action ), true, true ); } ?>
								...
								<?php } else {
									if ( empty( $this->product_data ) ) {
										...
										<div class="license-input product-id">
											...
											<label for="product_id" class="screen-reader-text"><?php esc_html_e( 'Product ID', 'atlasaidev' ); ?></label>
											<input class="regular-text" id="product_id" type="text"
											       value="<?php echo esc_attr( $this->license['product_id'] ); ?>"
											       placeholder="<?php esc_attr_e( 'Product ID', 'atlasaidev' ); ?>"
											       name="<?php echo esc_attr( $this->data_key ); ?>[product_id]"<?php if ( $wp_version < 5.9 ) { readonly( ( 'deactivate' == $action ), true, true ); } else { wp_readonly( ( 'deactivate' == $action ), true, true ); } ?>
											       autocomplete="off" required>
										</div>
										<!-- /.product-id-input -->
										<?php
										...
										<div class="license-input product-id">
											...
											<label for="product_id" class="screen-reader-text"><?php esc_html_e( 'Select License Type', 'atlasaidev' ); ?></label>
											<input type="hidden" name="<?php echo esc_attr( $this->data_key ); ?>[product_id]" value="<?php echo esc_attr( $this->client->getName() ); ?>">
											<select class="regular-text" id="product_id" name="<?php echo esc_attr( $this->data_key ); ?>[product_id]" <?php if ( $wp_version < 5.9 ) { readonly( ( 'deactivate' == $action ), true, true ); } else { wp_readonly( ( 'deactivate' == $action ), true, true ); } ?><?php disabled( ( 'deactivate' == $action ), true ); ?>>
												<option value="" disabled selected><?php esc_html_e( 'Select A License Type', 'atlasaidev' ); ?></option>
												<?php
							...
							<button type="submit" name="<?php echo esc_attr( $this->data_key); ?>[submit]" class="<?php printf( '%s-button', esc_attr( $action ) );?>"><?php
								'activate' == $action ? esc_html_e( 'Activate License', 'atlasaidev' ) : esc_html_e( 'Deactivate License', 'atlasaidev' );
							?></button>
							<a href="https://atlasaidev.com/my-account/" class="tta_btn" style="margin-left: 20px;font-size: 17px;line-height: 2.5;background:##1a4548" target="_blank"><?php esc_html_e( 'Manage License', 'atlasaidev' ); ?></a>
						</div>
					</form>
					<?php $this->show_active_license_info(); ?>
				</div>
			</div> <!-- /.atlasaidev-license-settings -->
			<?php do_action( 'after_atlasaidev_' . $this->client->getSlug() . '_license_section' ); ?>
		</div>
		<?php
	}
	...
	public function check_license_status() {
		...
		if ( $license ) {
			// check license.
			$response = $this->check();
			if ( isset($response['code']) && $response['code'] == "http_request_failed"){
				return [
					'success' => false,
					'error'   => "Request fail, network is unreachable",
	...
	private function licenses_style() {
		...
		<style>
			.atlasaidev-license-settings *{-webkit-box-sizing:border-box;box-sizing:border-box}
			.atlasaidev-license-settings{margin-top:20px;background-color:#fff;-webkit-box-shadow:0 3px 10px rgba(16,16,16,.05);box-shadow:0 3px 10px rgba(16,16,16,.05)}
			.atlasaidev-license-section{width:100%;min-height:1px;-webkit-box-sizing:border-box;box-sizing:border-box}
			.atlasaidev-license-title{background-color:#f8fafb;border-bottom:2px solid #eaeaea;display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-ms-flex-align:center;align-items:center;padding:10px 20px}
			.atlasaidev-license-title svg{width:30px;height:30px;fill:#0082bf}
			.atlasaidev-license-title span{font-size:17px;color:#444;margin-left:10px}
			.atlasaidev-license-details{padding:20px}
			.atlasaidev-license-details p{font-size:15px;margin:0 0 20px 0}
			...
			.license-input-fields button{margin-left:20px;color:#fff;font-size:17px;padding:8px;height:46px;background-color:#1a4548;border-radius:3px;cursor:pointer;-webkit-box-flex:0;-ms-flex:0 0 25%;flex:0 0 25%;max-width:25%;border:1px solid #1a4548}
	...
	private function show_active_license_info() {
		...
		<div class="active-license-info">
			<div class="single-license-info">
				<h3><?php esc_html_e( 'Status:', 'atlasaidev' ); ?></h3>
				<p class="<?php echo esc_attr( $status ); ?>"><?php 'active' == $status ? esc_html_e( 'Active', 'atlasaidev' ) : esc_html_e( 'Inactive', 'atlasaidev' ); ?></p>
			</div>
			<?php if ( false !== $unlimited ) { ?>
			<div class="single-license-info">
				<h3><?php esc_html_e( 'Activation Limit:', 'atlasaidev' ); ?></h3>
				<p class="active"><?php esc_html_e( 'Unlimited', 'atlasaidev' ); ?></p>
			</div>
			<div class="single-license-info">
				<h3><?php esc_html_e( 'Total Activation:', 'atlasaidev' ); ?></h3>
				<p class="active"><?php echo esc_attr( $activations ); ?></p>
			</div>
			<?php } else { ?>
				<div class="single-license-info">
					<h3><?php esc_html_e( 'Activation Remaining:', 'atlasaidev' ); ?></h3>
					<p class="<?php echo $remaining ? 'active' : 'inactive'; ?>"><?php
						if ( 'active' == $status ) {
							/* translators: 1: Remaining activation, 2: Total activation */
							printf( esc_html__( '%1$d out of %2$d', 'atlasaidev' ), esc_attr( $remaining ), esc_attr( $limit ) );
						} else {
							esc_html_e( 'N/A', 'atlasaidev' );
						}
					?></p>
				</div>
			<?php } ?>
			<div class="single-license-info">
				<h3><?php esc_html_e( 'Automatic Update:', 'atlasaidev' ); ?></h3>
				<p class="<?php echo esc_attr( $status ); ?>"><?php 'active' == $status ? esc_html_e( 'Enabled', 'atlasaidev' ) : esc_html_e( 'Disabled', 'atlasaidev' ); ?></p>
			</div>
		</div>
		<?php
	}
	...
	private function show_license_page_card_header() {
		?>
		<div class="atlasaidev-license-title">
			...
			<span><?php esc_html_e( 'Activate License', 'atlasaidev' ); ?></span>
		</div>
		<?php
	}
	...
	private function activate_client_license( $postData ) {
		if ( empty( $postData['license_key'] ) ) {
			$this->error = esc_html__('The license key field is required.', 'atlasaidev' );
			return;
		}
		...
		if ( $updateKey ) {
			...
			if ( ! $deactivate['success'] ) {
				...
				if ( $check['success'] && 'inactive' !== $check['status_check'] ) {
					$this->error = $deactivate['error'] ? $deactivate['error'] : esc_html__( 'Unknown error occurred.', 'atlasaidev' );
					return;
				}
			}
		}
		...
		if ( ! $response['success'] ) {
			$this->error = $response['error'] ? $response['error'] : esc_html__( 'Unknown error occurred.', 'atlasaidev' );
			return;
		}
		// Don't reset the key.
		// keep it, if the user renew subscription update the status and reactivate the plugin.
		...
		$this->setLicense( $license );
		if ( ! $updateKey ) {
			$this->success = esc_html__( 'License activated successfully.', 'atlasaidev' );
		} else {
			$this->success = esc_html__( 'License Updated successfully.', 'atlasaidev' );
		}
	}
	...
	private function deactivate_client_license() {
		if ( ! isset( $this->license['key'] ) || empty( $this->license['key'] ) ) {
			$this->error = esc_html__( 'License key not found.', 'atlasaidev' );
		} else {
		...
		$this->setLicense( [ 'instance' => '' ] );
		$this->success = esc_html__( 'License deactivated successfully.', 'atlasaidev' );
	}
	...
	public function should_show_notice_on_current_page() {
		...
		$allowed_pages  = [
			...
			admin_url().'admin.php?page=' . TTA_PRO_TEXT_DOMAIN
		];
